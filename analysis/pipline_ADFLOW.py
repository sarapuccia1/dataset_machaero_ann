...
# read airfoils names
namelist = []
f = open('/home/novermor/SARA/data_set_OFFICIAL/sampling/first_collection/set3.txt', 'r')
while True:
	filename = f.readline()
	if filename:
...
for iair in range(len(namelist)):
	...
	samples = np.load('/home/novermor/SARA/data_set_OFFICIAL/sampling/uiuc_sampled/' + namelist[iair])
	...
	output_directory = 'output/first_collection/' + namelist[iair].replace('.cgns', '')
	parser.add_argument("--output", type=str, default=output_directory)
	parser.add_argument("--gridFile", type=str, default=("/home/novermor/SARA/data_set_OFFICIAL/mesh/test_cgns/" + namelist[iair]))
				...
				if args.task == "analysis":
					...
					if comm.rank == 0:
						...
						if all_lower:
							...
							with open('dataset2.csv', 'a') as csvfile:
